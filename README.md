...
一个 Claude Code Skill，把你散落在各个项目目录下的 HTML 和 MD 文件，汇聚成一个有温度的个人产出中心。

*A Claude Code skill that turns your scattered HTML and MD files into a personal output center — with a beautiful dashboard, smart tagging, and weekly research insights.*
...
每次生成 HTML 或 MD 文件后，Skill 自动弹出收录提示，Claude 基于当前上下文推断项目名、主题、描述和标签，一键确认即可存档。
...
*Every time you generate an HTML or MD file, the skill automatically asks if you want to register it. Claude infers the project name, topic, description, and tags from context — one keypress to log it.*
...
**1. 将 Skill 复制到 Claude Code 的 skills 目录：**

```bash
cp -r html-to-center ~/.claude/skills/
```
...
Claude 会引导你配置项目根目录，并自动扫描已有文件批量导入。

*Claude will ask for your project root directory and set everything up, including scanning existing files for bulk import.*

---

## 日常使用 / Daily Usage

**生成任意 HTML 或 MD 文件后 / After generating any HTML or MD file:**
